...
public class A extends Pathfinding {

	public A(NodeMap map) {
		super(map);
	}

	@Override
	public void run(Node start, Node end) {
		PriorityQueue<Node> frontier = new PriorityQueue<Node>(new AComparator());
		...
		outer: while (!frontier.isEmpty()) {
			...
			while (siblings.hasNext()) {
				...
				sibling.setH(end);
				newCost = node.getValue() + sibling.getValue() + sibling.getH();
